...
async def cmd_start(message: types.Message):
    if message.from_user.id in blocked_users:
        await bot.send_message(message.from_user.id, "Вы уже прошли тестирование и не можете повторно пройти его.")
        return
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await Form.next()
    keyboard = back_button_keyboard()  # Добавляем кнопку "Назад"
...
async def process_city(message: types.Message, state: FSMContext):
    ...
    await Form.next()
    keyboard = yes_no_keyboard()  # Изменяем клавиатуру для следующего вопроса
...
async def process_spiritual_help_callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(spiritual_help=data)  # Сохраняем ответ в состояние

    if data == "Нет":
        await bot.send_message(callback_query.from_user.id,
                               "К сожалению, вы не подходите по интересам и профилю для участия в сообществе.")
...
@dp.callback_query_handler(
    lambda c: c.data in ['Медитация', 'Рейки', 'Йога', 'Целительство', 'Не указано мое направление'],
...
async def process_directions_callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await Form.next()
    keyboard = power_source_keyboard()  # Изменяем клавиатуру для следующего вопроса
...
@dp.callback_query_handler(lambda c: c.data in ['Природа', 'Вселенная', 'Энергия', 'Не указан источник моей силы'],
...
async def process_power_source_callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await bot.send_message(callback_query.from_user.id,
                               f"{data['name']}, Поздравляем! Теперь вы можете стать участником сообщества "
                               f"Проводников Света",
...
def yes_no_keyboard():
    ...
    keyboard.add(types.InlineKeyboardButton(text="Назад", callback_data="Назад"))  # Добавляем кнопку "Назад"
    return keyboard
...
def directions_keyboard():
    keyboard = types.InlineKeyboardMarkup()
    buttons = [
        types.InlineKeyboardButton(text="Медитация", callback_data="Медитация"),
        types.InlineKeyboardButton(text="Рейки", callback_data="Рейки"),
...
def power_source_keyboard():
    keyboard = types.InlineKeyboardMarkup()
    buttons = [
        types.InlineKeyboardButton(text="Природа", callback_data="Природа"),
        types.InlineKeyboardButton(text="Вселенная", callback_data="Вселенная"),
        types.InlineKeyboardButton(text="Энергия", callback_data="Энергия"),
...
async def new_chat_members(message: types.Message):
    for member in message.new_chat_members:
        ...
        welcome_message = (
            f"@{user_name}, приветствуем вас в сообществе ❤️\n"
